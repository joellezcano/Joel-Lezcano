...
class estudianteModel
{
    ...
    public function eliminar($idEstudiante)
    {
        $sql = 'DELETE FROM estudiantes WHERE idEstudiante = :idEstudiante';
        $statement = $this->PDO->prepare($sql);
        $statement->bindParam(':idEstudiante', $idEstudiante); // Enlazado correctamente el parámetro :idEstudiante.
        return ($statement->execute()) ? true : false;
    }
    ...
    {
        $sql = 'SELECT e.idEstudiante, e.nombre, e.apellido, e.idCurso, c.nombre as curso  FROM estudiantes  e join cursos c ON e.idCurso = c.idCurso ORDER BY idEstudiante DESC;';
        $statement = $this->PDO->prepare($sql);
    ...
    {
        $sql = 'SELECT e.idEstudiante, e.nombre, e.apellido, e.idCurso, c.nombre as curso  FROM estudiantes  e join cursos c ON e.idCurso = c.idCurso WHERE idEstudiante = :idEstudiante';
        $statement = $this->PDO->prepare($sql);
        $statement->bindParam(':idEstudiante', $idEstudiante);
